...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .loader-sub { color: #8b949e; font-size: 0.95rem; margin-bottom: 25px; }
        .spinner {
        ...
        .progress-bar-fill { width: 20%; height: 100%; background: #ff9f43; border-radius: 4px; transition: width 0.4s ease; }
        ...
        /* شاشة اللعب والتدريب */
        .game-screen {
            display: none; flex-direction: column; gap: 15px; padding: 15px 20px;
        }
        ...
        .board-wrapper {
            position: relative; width: 100%; background: #1e2736; padding: 8px;
            border-radius: 12px; box-shadow: 0 8px 20px rgba(0,0,0,0.4);
        }
        #board { width: 100%; border-radius: 6px; overflow: hidden; }
        #arrow-canvas {
        ...
        .instruction-box {
            ...
            padding: 10px; border-radius: 6px; font-size: 0.9rem; color: #d1d8e0; line-height: 1.4;
        }
        ...
        /* مودال الشعلة اليومية (Streak Modal) المطابق للصورة */
        #streak-modal {
            display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%;
            background: rgba(13, 17, 23, 0.92); z-index: 2000;
            flex-direction: column; justify-content: space-between; padding: 30px 20px; text-align: center;
        }
        .streak-modal-content { display: flex; flex-direction: column; align-items: center; gap: 20px; margin-top: 30px; }
        .streak-huge-icon { font-size: 5rem; animation: pulseFlame 1.5s infinite alternate; }
        @keyframes pulseFlame { 0% { transform: scale(1); } 100% { transform: scale(1.08); } }
        .streak-days-row { display: flex; justify-content: center; gap: 8px; margin-top: 15px; }
        .day-circle {
            width: 36px; height: 36px; border-radius: 50%; background: #1e2736;
            display: flex; flex-direction: column; align-items: center; justify-content: center;
            font-size: 0.75rem; font-weight: bold; color: #8b949e; border: 2px dashed #30363d;
        }
        .day-circle.active { background: #ff9f43; color: #121824; border: 2px solid #ff9f43; border-style: solid; }
        .day-label { font-size: 0.65rem; margin-bottom: 2px; }

        /* شاشة الأساسيات (Foundations Grid) */
        ...
        .piece-btn {
            ...
            border-radius: 10px; color: #fff; font-weight: bold; cursor: directo; text-align: center;
...
<body>
    ...
    <div id="loading-screen">
        ...
        <div class="loader-sub">جاري استرجاع تقدمك وحفظ متصفحك...</div>
        <div class="spinner"></div>
    </div>

    <!-- مودال الشعلة اليومية (Streak Daily Screen) -->
    <div id="streak-modal">
        <div></div>
        <div class="streak-modal-content">
            <h2 id="streakTopMsg" style="color:#fff; font-size: 1.1rem; margin:0;">Learn a new line or solve a puzzle to reach your streak!</h2>
            <div class="streak-huge-icon">🔥</div>
            <h1 id="streakCountBig" style="color: #ff9f43; font-size: 2.2rem; margin:0;">1 Day Streak</h1>
            
            <div class="streak-days-row" id="streakDaysContainer">
                <!-- سيتم تعبئتها ديناميكياً بالأيام -->
            </div>
        </div>
    ...
    <div class="app-container">
        ...
        <!-- تبويب الافتتاحيات (Openings) -->
        <div id="tab-openings" class="tab-content active">
            <!-- بطاقة الإهداء لأخي مالك -->
            <div class="gift-banner">
                <h3>🎁 إهداء خاص</h3>
                <p>هذا التطبيق مهداة إلى أخي الصغير <strong>مالك عنتر</strong> ❤️ ليتعلم أسرار الشطرنج باحتراف!</p>
            </div>

            <!-- نظام الشارات (Rank & Status) -->
            <div class="rank-card">
        ...
        <!-- شاشة اللعب والتدريب (افتتاحيات أو أساسيات) -->
        <div id="game-screen" class="game-screen">
            <div class="game-top-bar">
                <button class="back-dash-btn" onclick="closeOpeningGame()">⬅ القائمة</button>
                <h3 id="openingGameTitle" style="color: #ff9f43; margin:0; font-size: 1rem;">الافتتاحية</h3>
            </div>
        ...
        <!-- تبويب القواعد الأساسية (Foundations) مع أزرار منفصلة لكل قطعة -->
        <div id="tab-foundations" class="tab-content">
            ...
            <div class="pieces-grid">
                <div class="piece-btn" onclick="startFoundationPiece('king')">
                    👑 <span>حركة الملك</span>
                </div>
                <div class="piece-btn" onclick="startFoundationPiece('queen')">
                    ♕ <span>حركة الوزير</span>
                </div>
                <div class="piece-btn" onclick="startFoundationPiece('rook')">
                    ♜ <span>حركة القلعة</span>
                </div>
                <div class="piece-btn" onclick="startFoundationPiece('bishop')">
                    ♝ <span>حركة الفيل</span>
                </div>
                <div class="piece-btn" onclick="startFoundationPiece('knight')">
                    ♞ <span>حركة الحصان</span>
                </div>
                <div class="piece-btn" onclick="startFoundationPiece('pawn')">
                    ♟ <span>حركة البيدق</span>
                </div>
            </div>
        </div>

        <!-- تبويب النهايات (Endgames) -->
        ...
        <div id="tab-settings" class="tab-content">
            ...
            <div class="terminal-box">
                > System init: <span>ChessCraft v3.1 (Persistent)</span><br>
            ...
            <div class="gift-banner" style="margin-top: 10px;">
                <h3>✨ صُنع بكل حب</h3>
                <p>تم حفظ تقدمك تلقائياً في هذا المتصفح بواسطة <strong>Mohamed Antar</strong> لصالح شركة <strong>MK CREATIVE AGENCY</strong>.</p>
            </div>
        </div>
    ...
    <script>
        // نظام إدارة التخزين المحلي (LocalStorage & Streak Logic)
        let appData = {
            streak: 1,
            lastLoginDate: "",
            completedOpenings: []
        ...
        function loadUserData() {
            ...
            if (saved) {
                ...
                let lastDate = new Date(appData.lastLoginDate);
                let currentDate = new Date(today);
                let diffTime = currentDate - lastDate;
                let diffDays = diffTime / (1000 * 60 * 60 * 24);

                if (diffDays === 1) {
                    // فتح في اليوم التالي مباشرة -> زيادة الشعلة
                    appData.streak += 1;
                    appData.lastLoginDate = today;
                } else if (diffDays > 1) {
                    // نسي يفتح التطبيق يوم أو أكثر -> رجوع للصفر
                    appData.streak = 1;
                    appData.lastLoginDate = today;
                }
                // إذا فتح في نفس اليوم لا نغير الشعلة
            } else {
                appData.streak = 1;
                appData.lastLoginDate = today;
            }
        ...
        function updateUIStats() {
            ...
            // تحديث الشارات والتقدم بناءً على الافتتاحيات المنتهية
            let completedCount = appData.completedOpenings.length;
            let percent = (completedCount / 5) * 100;
            ...
            if (completedCount >= 5) {
                ...
                document.getElementById('nextRankText').innerText = "Next: Master";
            } else {
            ...
            // تحديث علامات الافتتاحيات المكتملة
        ...
        function buildStreakDaysUI() {
            ...
            let todayIndex = (new Date().getDay() + 6) % 7; // ترتيبي من الإثنين
        ...
        window.addEventListener('load', function() {
            loadUserData();
            // إظهار شاشة الشعلة عند الفتح اليومي لأول مرة
            setTimeout(() => {
                openStreakModal();
        ...
        var openings = {
            ...
            caroKann: {
                ...
                steps: [
                    ...
                    { move: "d5", from: "d7", to: "d5", hint: "اضضرب الوسط واحتل المربع بقوة." }
                ]
            }
        };

        // بيانات أساسيات القطع المنفصلة مع أسهم توجيهية
        ...
        function startFoundationPiece(pieceKey) {
            ...
            board.position(pieceData.fen);
            board.orientation('white');

            document.querySelectorAll('.tab-content').forEach(t => t.classList.remove('active'));
            document.getElementById('game-screen').style.display = 'flex';
            document.getElementById('openingGameTitle').innerText = pieceData.name;
            document.getElementById('instructionText').innerText = "هكذا تتحرك هذه القطعة على الرقعة كما توضح الأسهم الخضراء.";
        ...
        function onDragStart (source, piece, position, orientation) {
            if (isFoundationMode) return false; // الأساسيات للعرض فقط
        ...
        function onDrop (source, target) {
            ...
            let isBlackOpening = (currentLine.key === 'sicilian' || currentLine.key === 'caroKann');
